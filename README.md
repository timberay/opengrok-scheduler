...
- **Crash-Safe Per-Service Lock**: On top of the DB-level guard, the dispatched subshell holds an exclusive `flock` on `${DB_PATH}.svc.<container>.lock` via an inherited file descriptor. Because file locks live on the open-file-description and not on the parent process, an indexing subshell that outlives its parent scheduler (e.g. after a hard crash between INSERT and pid UPDATE) keeps the lock alive while it runs. A fresh scheduler that tries to re-dispatch the same service finds the lock taken, logs `Process check skip: external svc-lock held for <container>...`, and waits — so two indexing processes against the same target cannot run concurrently even when the DB row has already been swept to `ORPHANED` by recovery.
...
./tests/test_svc_lock_orphan_defense.sh    # Check the per-service flock prevents duplicate dispatch when an orphan subshell from a prior crash still holds the lock
